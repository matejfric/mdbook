...
Each Token  is a reference type with a single
numerical value. The machine supports adding tokens
and, when it does, it returns a memento representing
...
a token is fed in as a reference and its value
is subsequently changed on that reference,
...
class TokenMachine:
    ...
    def add_token(self, token):
        ...
        # Notice that we are returning a memento by value.
        # We do not want to return a reference to our internal state.
...
class Evaluate(unittest.TestCase):
    ...
    def test_fiddled_token(self):
        ...
        token.value = 333  # <-- here's the fiddle
